...
class RectifiedLogistic(GammaModel):
    ...
    def _model(self, intensity, features, response_obs=None):
        ...
        logger.debug(f"Intensity: {intensity.shape}")
        logger.debug(f"Features: {features.shape}")

        logger.debug(f"n_features: {n_features}")
        logger.debug(f"n_features: {n_features.shape}")
        ...
        feature0 = features[..., 0]
        feature1 = features[..., 1]
        logger.debug(f"feature0: {feature0.shape}")
        logger.debug(f"feature1: {feature1.shape}")
...
class ReLU(GammaModel):
    ...
    def _model(self, intensity, features, response_obs=None):
        ...
        with numpyro.plate(site.n_response, self.n_response):
            with numpyro.plate(site.n_data, n_data, subsample_size=5):
                # Model
                mu = numpyro.deterministic(
                    site.mu,
                    F.relu(
