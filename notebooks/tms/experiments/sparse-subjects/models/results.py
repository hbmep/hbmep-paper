...
def main():
    ...
    logger.info(sim_obs.shape)
    logger.info(sim_a.shape)
    draw = 136
    logger.info(sim_a[draw, 7, :, 0])
    logger.info(
        f"{simulation_posterior_predictive[site.g_1][draw, 6, :, 0]}, {simulation_posterior_predictive[site.b][draw, 6, :, 0]}"
    )
    ind = simulator
    logger.info(simulation_posterior_predictive[site.g_1][draw, 7, :, 0])
    return
    # model = "nhb_model"
    # model = "hb_model"
    ...
    # draws_space = [i for i in range(0, 96)]
    # # draws_space += [i for i in range(130, 137)]
    # # draws_space = [i for i in range(135, 136)]
    # draws_space += [i for i in range(100, 430)]
