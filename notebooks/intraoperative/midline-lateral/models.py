...
class HierarchicalBayesianModel(GammaModel):
    ...
    def _model(self, intensity, features, response_obs=None):
        ...
        n_fixed = 1
        n_delta = n_features[1] - 1     # Equals 1

        # Fixed
        a_fixed_loc_loc = numpyro.sample(
            "a_fixed_loc_loc", dist.TruncatedNormal(5., 10., low=0)
        )
        a_fixed_loc_scale = numpyro.sample(
            "a_fixed_loc_scale", dist.HalfNormal(10.)
        )
        ...
        with numpyro.plate(site.n_response, self.n_response):
            with numpyro.plate("n_fixed", n_fixed):
                a_fixed_loc = numpyro.sample(
                    "a_fixed_loc", dist.TruncatedNormal(a_fixed_loc_loc, a_fixed_loc_scale, low=0)
                )
                a_fixed_scale = numpyro.sample(
        ...
        # Random Effects (Delta)
        a_random_loc_loc = numpyro.sample(
            "a_random_loc_loc", dist.Normal(0., 10.)
        )
        a_random_loc_scale = numpyro.sample(
            "a_random_loc_scale", dist.HalfNormal(10.)
        )
        a_random_scale_scale = numpyro.sample(
            "a_random_scale_scale", dist.HalfNormal(10.)
        )

        with numpyro.plate(site.n_response, self.n_response):
            with numpyro.plate("n_random", n_random):
                a_random_loc = numpyro.sample(
                    "a_random_loc", dist.Normal(a_random_loc_loc, a_random_loc_scale)
                )
                a_random_scale = numpyro.sample(
                    "a_random_scale", dist.HalfNormal(a_random_scale_scale)
                )
                ...
                with numpyro.plate(site.n_features[0], n_features[0]):
                    a_random_raw = numpyro.sample(
                        "a_random_raw", dist.Normal(0., 1.)
                    )
                    a_random = numpyro.sample(
                        "a_random", dist.Normal(a_random_mean, a_random_scale)
                    )
