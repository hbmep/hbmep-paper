        ...
        with numpyro.plate(site.n_response, self.n_response):
            with numpyro.plate(site.n_features[1], n_features[1]):
                with numpyro.plate(site.n_features[0], n_features[0]):
                    ...
                    # # Priors
                    # a = numpyro.sample(site.a, dist.TruncatedNormal(5., 5., low=0))
                    ...
                    # H = numpyro.sample(site.H, dist.HalfNormal(10.))

                    # c_1 = numpyro.sample(site.c_1, dist.HalfNormal(5.))
                    # c_2 = numpyro.sample(site.c_2, dist.HalfNormal(.5))
        ...
        with numpyro.plate(site.n_response, self.n_response):
            with numpyro.plate(site.n_features[1], n_features[1]):
                with numpyro.plate(site.n_features[0], n_features[0]):
                    ...
                    H = numpyro.sample(site.H, dist.Uniform(0., 100.))
...
class NelderMeadOptimization(BoundConstrainedOptimization):
    ...
    def __init__(self, config: Config):
        ...
        self.bounds = [(1e-9, 50.), (1e-9, 10), (1e-9, 10), (1e-9, 10), (1e-9, 50)]
        self.informed_bounds = [(2, 8), (1e-3, 5), (1e-4, .1), (1e-2, 5), (.5, 10)]
        self.num_reinit = 100
        self.n_jobs = -1
...
class HBnull(GammaModel):
    NAME = "hierarchical_bayesian_model"

    def __init__(self, config: Config):
        super(HBnull, self).__init__(config=config)
    ...
    def _model(self, intensity, features, response_obs=None):
        ...
        feature1 = features[..., 1]
        response = features[..., 2]
        n_response = response.max() + 1

        if response_obs is not None:
            response_obs = response_obs.reshape(-1,)
        ...
        with numpyro.plate(site.n_data, n_data):
            ...
            mu = numpyro.deterministic(
                site.mu,
                S.rectified_logistic(
                    x=intensity.reshape(-1,),
                    a=a[feature0, feature1, response],
                    b=b[feature0, feature1, response],
                    L=L[feature0, feature1, response],
                    ell=ell[feature0, feature1, response],
                    H=H[feature0, feature1, response],
                    eps=EPS
                )
            )
            # print(f"mu: {mu.shape}")
            beta = numpyro.deterministic(
                ...
                self.rate(
                    mu,
                    c_1[feature0, feature1, response],
                    c_2[feature0, feature1, response]
                )
            )
