...
from utils import generate_pulses_map
from constants__accuracy import (
    TOML_PATH,
    BUILD_DIR as ACCURACY_BUILD_DIR,
...
SIMULATION_DF_PATH = os.path.join(ACCURACY_BUILD_DIR, SIMULATION_DF)
SIMULATION_PPD_PATH = os.path.join(ACCURACY_BUILD_DIR, INFERENCE_FILE)
...
def main(draws_space, n_subjects_space, models, n_jobs=-1):
    ...
    pulses_map = generate_pulses_map(N_PULSES_SPACE)
    ...
    def run_experiment(
        ...
        match M.NAME:
            case (
                ...
                # temp = df.groupby(simulator.features)[simulator.intensity].nunique().unique().tolist()
                # assert len(temp) == 1
                # assert temp[0] == n_pulses
                # logger.info(f"draw: {draw}, n_subjects: {n_subjects}: OK")
