...
class HB(BaseModel):
    ...
    def _model(self, subject_ind, y_obs=None):
        ...
        a_loc_scale_scale = numpyro.sample("a_loc_scale_scale", dist.HalfNormal(50.))

        with numpyro.plate("n_tests", n_tests):
            a_loc_scale = numpyro.sample(
                "a_loc_scale", dist.HalfCauchy(a_loc_scale_scale)
            )
...
class HB2(BaseModel):
    ...
    def _model(self, subject_ind, y_obs=None):
        ...
        with numpyro.plate("n_tests", n_tests):
            # a_loc_scale = numpyro.sample("a_loc_scale", dist.HalfCauchy(50.))

            # a_low_raw = numpyro.sample("a_low_raw", dist.Normal(0., 1.))
            # a_loc_ = numpyro.deterministic("a_loc_", -10 + a_low_raw)       # ~ Normal(-10, 1)
            # a_loc = numpyro.deterministic("a_loc", 10 + a_loc_ * a_loc_scale)       # ~ Normal(0, a_loc_scale)
            a_loc = numpyro.sample("a_loc", dist.Normal(0., 50.))
    ...
    #     with numpyro.plate("n_tests", n_tests):
    #         a_loc_scale = numpyro.sample(
    #             "a_loc_scale", dist.TruncatedNormal(0., 50., low=0)
    #         )
    #         a_loc_raw = numpyro.sample("a_loc_raw", dist.Normal(5., 1.))
    #         a_loc = numpyro.deterministic("a_loc", -5 + a_loc_scale * a_loc_raw)
    #         # a_loc = numpyro.sample(
    #         #     "a_loc", dist.Normal(0., a_loc_scale)
    #         # )
    ...
    #     # a_loc_scale_scale = numpyro.sample("a_loc_scale_scale", dist.HalfNormal(5.))
    #     a_loc_scale = numpyro.deterministic("a_loc_scale", jnp.array([10, .5, .5, .5]))

    #     with numpyro.plate("n_tests", n_tests):
    #         # a_loc_scale = numpyro.sample(
    #         #     "a_loc_scale", dist.HalfNormal(a_loc_scale_scale)
    #         # )
    #         a_loc = numpyro.sample(
    #             "a_loc", dist.Normal(0., a_loc_scale)
    #         )
