...
def main(experiments_dir, ind, draws_space, n_subjects_space, models):
            ...
            for M in models:
                for n_subjects in n_subjects_space:
                    if "HB" in M.NAME:
                        ...
                        #     | ((diff < 0).mean(axis=0) > 1 - (SIGNIFICANCE_LEVEL / 2))
                        # )
                        hdi = hpdi(diff, prob=1 - (SIGNIFICANCE_LEVEL / 4), axis=0)
                        ...
                        pr = stats.wilcoxon(
                            x=a, alternative="two-sided", axis=0
                        ).pvalue
                        # pr = stats.ttest_1samp(
                        #     a=a, alternative="two-sided", popmean=0, axis=0
                        # ).pvalue
    ...
    if t1_ind != []:
        ...
        print(arr[..., t1_ind].any(axis=-1).mean(axis=0) - stats.sem(arr[..., t1_ind].any(axis=-1), axis=0))
        print(arr[..., t1_ind].any(axis=-1).mean(axis=0) - 1.96 * stats.sem(arr[..., t1_ind].any(axis=-1), axis=0))

    # if power_ind != []:
    #     print("\nCorrected")
    #     print(correct_reject[..., power_ind].mean(axis=0)[..., 0])

    # if t1_ind != []:
    #     print()
    #     print(correct_reject[..., t1_ind].any(axis=-1).mean(axis=0))
    #     # print(arr[..., t1_ind].any(axis=-1).mean(axis=0) - stats.sem(arr[..., t1_ind].any(axis=-1), axis=0))
    #     # print(arr[..., t1_ind].any(axis=-1).mean(axis=0) - 1.96 * stats.sem(arr[..., t1_ind].any(axis=-1), axis=0))
