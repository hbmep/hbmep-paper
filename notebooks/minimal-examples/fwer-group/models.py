...
class HB(BaseModel):
    ...
    def _model(self, subject_ind, group_ind, y_obs=None):
        ...
        a_loc_fixed_loc = numpyro.sample(
            "a_loc_fixed_loc", dist.Normal(50., 50.)
        )
        ...
        with numpyro.plate("n_tests", n_tests):
            with numpyro.plate("n_fixed", n_fixed):
                a_loc_fixed_raw = numpyro.sample("a_loc_fixed_raw", dist.Normal(0., 1.))
                a_loc_fixed = numpyro.deterministic(
                    "a_loc_fixed", a_loc_fixed_loc + a_loc_fixed_raw * a_loc_fixed_scale
                )
                # a_loc_fixed = numpyro.sample("a_loc_fixed", dist.Normal(a_loc_fixed_loc, a_loc_fixed_scale))

        a_loc_delta_scale_scale = numpyro.sample("a_loc_delta_scale_scale", dist.HalfNormal(50.))

        with numpyro.plate("n_tests", n_tests):
            with numpyro.plate("n_delta", n_delta):
                a_loc_delta_scale_raw = numpyro.sample("a_loc_delta_scale_raw", dist.HalfCauchy(1.))
                a_loc_delta_scale = numpyro.deterministic(
                    "a_loc_delta_scale", a_loc_delta_scale_raw * a_loc_delta_scale_scale
                )
                # a_loc_delta_scale = numpyro.sample("a_loc_delta_scale", dist.HalfCauchy(a_loc_delta_scale_scale))

                a_loc_delta_raw = numpyro.sample("a_loc_delta_raw", dist.Normal(0., 1.))
                a_loc_delta = numpyro.deterministic(
                    "a_loc_delta", a_loc_delta_raw * a_loc_delta_scale
                )
                # a_loc_delta = numpyro.sample("a_loc_delta", dist.Normal(0., a_loc_delta_scale))
        ...
        with numpyro.plate("n_tests", n_tests):
            with numpyro.plate("n_data", n_data):
                numpyro.sample(
                    "obs",
                    dist.Normal(a[subject_ind, group_ind], scale),
                    obs=y_obs
                )
...
class HB2(BaseModel):
    ...
    def _model(self, subject_ind, group_ind, y_obs=None):
        ...
        # a_loc_delta_scale_scale = numpyro.sample("a_loc_delta_scale_scale", dist.HalfNormal(50.))
        a_loc_delta_loc = numpyro.sample("a_loc_delta_loc", dist.Normal(0., 50.))
        a_loc_delta_scale = numpyro.sample("a_loc_delta_scale", dist.HalfNormal(50.))

        with numpyro.plate("n_tests", n_tests):
            with numpyro.plate("n_delta", n_delta):
                # a_loc_delta_scale_raw = numpyro.sample("a_loc_delta_scale_raw", dist.HalfNormal(1.))
                # a_loc_delta_scale = numpyro.deterministic(
                #     "a_loc_delta_scale", a_loc_delta_scale_raw * a_loc_delta_scale_scale
                # )
                # a_loc_delta_scale = numpyro.sample("a_loc_delta_scale", dist.HalfCauchy(a_loc_delta_scale_scale))
                a_loc_delta = numpyro.sample("a_loc_delta", dist.Normal(a_loc_delta_loc, a_loc_delta_scale))
...
class NonBaseModel(BaseModel):
    ...
    def run(
        ...
        **kwargs
        ...
        features = ["subject_ind", "group_ind"]
        ...
        def body_run(combination, response, destination_path):
            ...
            group_ind_ = df_["group_ind"].values
            y_obs_ = df_[[response]].values

            mcmc, posterior_samples = BaseModel.run(
                self, subject_ind_, group_ind_, y_obs=y_obs_, **kwargs
            )
