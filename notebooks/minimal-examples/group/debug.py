            ...
            for M in models:
                for n_subjects in n_subjects_space:
                    if "HB" in M.NAME:
                        ...
                        fig, axes = plt.subplots(nr, nc, squeeze=False, constrained_layout=True, figsize=(12, 4), sharex="row", sharey="row")
                        for j in range(4):
                            samples = diff[:, j]
                            sns.kdeplot(samples, ax=axes[0, j])
                            samples = samples.reshape(4, -1)
                            for i in range(4):
                                sns.lineplot(x=np.arange(samples.shape[-1]), y=samples[i, :], ax=axes[1, j])
...
if __name__ == "__main__":
    ...
    experiments_dir = f"/home/vishu/repos/hbmep-paper/reports/minimal-examples/fwer-group/{key}/experiments"

    # key, ind = "strong2", ([0, 1, 2, 3], [])
    # experiments_dir = f"/home/vishu/repos/hbmep-paper/reports/minimal-examples/fwer-group/{key}/experiments"
