...
dest = "/home/vishu/logs/tms-mixture-model.log"
logging.basicConfig(
    format=FORMAT,
    level=logging.INFO,
    handlers=[
    ...
    def _model(self, features, intensity, response_obs=None):
        ...
        with numpyro.plate(site.n_response, self.n_response):
            ...
            a_mean_global_mean = numpyro.sample(
                "a_mean_global_mean", dist.TruncatedNormal(50., 20., low=0)
            )
            a_mean_global_scale = numpyro.sample(
                "a_mean_global_scale", dist.HalfNormal(50.)
            )
            a_scale_global_scale = numpyro.sample(
                "a_scale_global_scale", dist.HalfNormal(50.)
            )
            ...
            with numpyro.plate(site.n_features[1], n_features[1]):
                ...
                a_mean = numpyro.sample(
                    "a_mean",
                    dist.TruncatedNormal(a_mean_global_mean, a_mean_global_scale, low=0)
                )
                a_scale_raw = numpyro.sample("a_scale_raw", dist.HalfNormal(scale=1.))
                a_scale = numpyro.deterministic("a_scale", jnp.multiply(a_scale_global_scale, a_scale_raw))

                b_scale_raw = numpyro.sample("b_scale_raw", dist.HalfNormal(1.))
                b_scale = numpyro.deterministic("b_scale", jnp.multiply(b_scale_global_scale, b_scale_raw))

                v_scale_raw = numpyro.sample("v_scale_raw", dist.HalfNormal(1.))
                v_scale = numpyro.deterministic("v_scale", jnp.multiply(v_scale_global_scale, v_scale_raw))

                L_scale_raw = numpyro.sample("L_scale_raw", dist.HalfNormal(1.))
                L_scale = numpyro.deterministic("L_scale", jnp.multiply(L_scale_global_scale, L_scale_raw))

                ell_scale_raw = numpyro.sample("ell_scale_raw", dist.HalfNormal(1.))
                ell_scale = numpyro.deterministic("ell_scale", jnp.multiply(ell_scale_global_scale, ell_scale_raw))

                H_scale_raw = numpyro.sample("H_scale_raw", dist.HalfNormal(1.))
                H_scale = numpyro.deterministic("H_scale", jnp.multiply(H_scale_global_scale, H_scale_raw))

                g_1_scale_raw = numpyro.sample("g_1_scale_raw", dist.HalfNormal(1.))
                g_1_scale = numpyro.deterministic("g_1_scale", jnp.multiply(g_1_scale_global_scale, g_1_scale_raw))

                g_2_scale_raw = numpyro.sample("g_2_scale_raw", dist.HalfNormal(1.))
                ...
                with numpyro.plate(site.n_features[0], n_features[0]):
                    ...
                    b_raw = numpyro.sample("b_raw", dist.HalfNormal(1.))
                    b = numpyro.deterministic(site.b, jnp.multiply(b_scale, b_raw))

                    v_raw = numpyro.sample("v_raw", dist.HalfNormal(1.))
                    v = numpyro.deterministic(site.v, jnp.multiply(v_scale, v_raw))

                    L_raw = numpyro.sample("L_raw", dist.HalfNormal(1.))
                    L = numpyro.deterministic(site.L, jnp.multiply(L_scale, L_raw))

                    ell_raw = numpyro.sample("ell_raw", dist.HalfNormal(1.))
                    ell = numpyro.deterministic(site.ell, jnp.multiply(ell_scale, ell_raw))

                    H_raw = numpyro.sample("H_raw", dist.HalfNormal(1.))
                    H = numpyro.deterministic(site.H, jnp.multiply(H_scale, H_raw))

                    g_1_raw = numpyro.sample("g_1_raw", dist.HalfCauchy(1.))
                    g_1 = numpyro.deterministic(site.g_1, jnp.multiply(g_1_scale, g_1_raw))

                    g_2_raw = numpyro.sample("g_2_raw", dist.HalfCauchy(1.))
...
def main():
    ...
    config.BUILD_DIR = os.path.join(config.BUILD_DIR, "model-comparison", "mixture-model", "response-prior-on-a")
