...
def main():
    ...
    draws_space = [i for i in range(0, 26)]
    draws_space += [i for i in range(27, 45)]
    draws_space += [i for i in range(46, 55)]
    draws_space += [i for i in range(56, 100)]
    # draws_space = [i for i in range(66, 70)]
    ...
    for model in models:
        ...
        mae = np.array(mae).reshape(len(n_subjects_space), len(draws_space))
        mse = np.array(mse).reshape(len(n_subjects_space), len(draws_space))
        prob = np.array(prob).reshape(len(n_subjects_space), len(draws_space))
    ...
    dest = f"/home/vishu/repos/hbmep-paper/reports/paper/tms/experiments/sparse-subjects/hb-simulate-data/a_random_mean_-5_a_random_scale_2/both.png"
