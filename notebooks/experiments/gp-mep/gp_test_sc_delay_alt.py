...
base_rng_key = random.PRNGKey(0)  # `seed` is an integer seed of your choice
N = 64 # number of stim intensities
rng_keys = random.split(base_rng_key, N)
...
#     predictive = Predictive(model, ps, params=params, num_samples=1000)
...
def modified_kernel(X, t1, t2, var, length, noise, jitter=1.0e-6, include_noise=True):
    K = kernel(t1, t2, var, length, noise, jitter=1.0e-6, include_noise=True)
    K_stack = jnp.tile(jnp.expand_dims(K, axis=2), (1, 1, len(X)))

    return K_stack


def model(X, t, Y=None):
    # Sample hyperparameters as before
    ...
    # Generate a 3D covariance matrix based on X
    kernel_bio1 = modified_kernel(X, t, t, 1.0, length_bio1, noise_bio1)

    # Define other parameters and distributions as before
    ...
    mu_bio1 = F.relu(X.flatten()[:, None], a_bio1, b_bio1, L)
    ...
    draws_bio1 = draws_bio1.squeeze()  # This changes its shape from (64, 1) to (64,)

    # Sample from the MultivariateNormal for each X row
    def sample_for_row(kernel_slice, draw, rng_key):
        # gp_sample = numpyro.sample("gp_sample", dist.MultivariateNormal(loc=jnp.zeros(t.shape[0]), covariance_matrix=kernel_slice))
        gp_sample = dist.MultivariateNormal(loc=jnp.zeros(t.shape[0]), covariance_matrix=kernel_slice).sample(rng_key)

        return gp_sample * draw
    gp_samples = vmap(sample_for_row, in_axes=(2, 0, 0))(kernel_bio1, draws_bio1, rng_keys)

    obs_noise = numpyro.sample("obs_noise", dist.HalfNormal(scale=1))
    numpyro.sample("Y", dist.Normal(gp_samples, obs_noise), obs=Y)


def kernel(t1, t2, var, length, noise, jitter=1.0e-6, include_noise=True):
    deltaXsq = jnp.power((t1[:, None] - t2) / length, 2.0)
    k = var * jnp.exp(-0.5 * deltaXsq)
    if include_noise:
        k += (noise + jitter) * jnp.eye(t1.shape[0])
    return k
...
if framework == "MCMC":
    ...
    n_steps = int(2e4)
    svi_state = svi.init(base_rng_key, X, t, Y)
    ...
    predictive = Predictive(model, ps, params=svi.get_params(svi_state), num_samples=1000)
    samples = predictive(random.PRNGKey(1), X, t)
