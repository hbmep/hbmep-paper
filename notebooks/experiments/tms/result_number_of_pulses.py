...
from exp_number_of_pulses import fix_rng
from exp_number_of_pulses import EXPERIMENT_NAME, N_DRAWS, N_REPEATS, N_SUBJECTS

plt.rcParams['svg.fonttype'] = 'none'

logger = logging.getLogger(__name__)
FORMAT = "%(asctime)s - %(name)s - %(levelname)s - %(message)s"
dest = "/home/vishu/logs/pulses-experiment-results.log"
logging.basicConfig(
    format=FORMAT,
    level=logging.INFO,
    handlers=[
...
def main():
        ...
        for draw in draws_space:
            for seed in seeds_for_generating_subjects:
                ...
                for M in models:
                    ...
                    if M.NAME in ["hbm"]:
                        dir = os.path.join(simulator.build_dir, EXPERIMENT_NAME, draw_dir, n_pulses_dir, seed_dir, M.NAME)
                        ...
                        for subject in subjects:
                            sub_dir = f"p{subject}"
                            dir = os.path.join(simulator.build_dir, EXPERIMENT_NAME, draw_dir, n_pulses_dir, seed_dir, M.NAME, sub_dir)
    ...
    mae = np.array(mae).reshape(len(n_pulses_space), len(draws_space), len(seeds_for_generating_subjects), len(models))
    mse = np.array(mse).reshape(len(n_pulses_space), len(draws_space), len(seeds_for_generating_subjects), len(models))
    prob = np.array(prob).reshape(len(n_pulses_space), len(draws_space), len(seeds_for_generating_subjects), len(models))
    ...
    fig.align_xlabels()
    fig.align_ylabels()
    dest = os.path.join(simulator.build_dir, "result_number_of_pulses.svg")
    fig.savefig(dest, dpi=600)
    dest = os.path.join(simulator.build_dir, "result_number_of_pulses.png")
    fig.savefig(dest, dpi=600)
