...
TOML_PATH = "/home/mcintosh/Local/gitprojects/hbmep-paper/configs/experiments/real-time.toml"
...
def fit_lookahead_wrapper(simulation_df_future, cand_y_at_x):
    simulation_df_future.iloc[-1, simulation_df_future.columns.get_loc("PKPK_APB")] = cand_y_at_x
        ...
        for ix in range(N_max):
            ...
            if ix < 1:
                ...
                for ix_future in range(len(list_candidate_intensities)):
                    ...
                    entropy_list = []
                    # the order of entropy_list is important... should make 100% sure it is preserved
                    # maybe change to single chain - but only for this rather than everything
                    with Parallel(n_jobs=-1) as parallel:
                        entropy_list = parallel(
                            delayed(fit_lookahead_wrapper)(simulation_df_future, candidate_y_at_this_x[ix_sample])
                            for ix_sample in range(N)
                        )

                    # for ix_sample in range(N):
                    #     entropy = fit_lookahead_wrapper(simulation_df_future, candidate_y_at_this_x[ix_sample])
