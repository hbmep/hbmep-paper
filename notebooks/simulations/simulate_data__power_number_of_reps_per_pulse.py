...
def main(a_delta_loc, a_delta_scale, build_dir):
    ...
    simulation_df = simulator.make_prediction_dataset(
        ...
        min_intensity=0,
        max_intensity=100,
        num_points=REPS_TOTAL_PULSES
    )
    simulation_df = (
        pd.concat([simulation_df] * REPS_TOTAL_REPS, ignore_index=True)
        .reset_index(drop=True)
        .copy()
    )
    arr = []
    for i in range(0, REPS_TOTAL_REPS):
        arr += [i] * TOTAL_SUBJECTS * REPS_TOTAL_PULSES * 2
...
if __name__ == "__main__":
    ...
    main(-5., 2.5, REPS_SIMULATE_DATA_WITH_EFFECT_DIR)

    # Simulate data with no effect
    main(0., 2.5, REPS_SIMULATE_DATA_WITH_NO_EFFECT_DIR)
