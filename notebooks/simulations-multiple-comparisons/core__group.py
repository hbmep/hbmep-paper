...
from utils__group import _generate_simulation_data_dirs
    ...
    def run_experiment(
        ...
        match M.NAME:
            case "hierarchical_bayesian_model":
                ...
                # A subject should belong to exactly one group
                df[simulator.features[0]] = (
                    df[simulator.features[1]].astype(str)
                    + "__"
                    + df[simulator.features[0]].astype(str)
                )
            ...
            case "non_hierarchical_bayesian_model" | "maximum_likelihood_model":
                for subject in range(n_subjects):
                    for intervention in range(2):
                        for response_ind, response in enumerate(simulator.response):
                            sub_dir = f"subject{subject}"
                            intervention_dir = f"inter{intervention}"
                            response_dir = f"response{response_ind}"
...
if __name__ == "__main__":
    ...
    key = "with_effect"
    _, simulation_data_dir = simulation_data_dirs[key]
    main(
