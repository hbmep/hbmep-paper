        ...
        with numpyro.plate(site.n_response, self.n_response, dim=-1):
            with numpyro.plate("n_baseline", n_baseline, dim=-2):
                ...
                with numpyro.plate(site.n_subject, n_subject, dim=-3):
                    ...
                    g_1_baseline = numpyro.sample("g_1_baseline", dist.Exponential(0.01))
                    g_2_baseline = numpyro.sample("g_2_baseline", dist.Exponential(0.01))
...
class ALL_IN(Baseline):
    LINK = "all_in"

    def __init__(self, config: Config):
        super(ALL_IN, self).__init__(config=config)
        self.combination_columns = self.features + [self.subject]
...
class HBIterated(Baseline):
    LINK = "hb_iterated"

    def __init__(self, config: Config):
        super(HBIterated, self).__init__(config=config)
        self.combination_columns = self.features + [self.subject]
...
if __name__ == "__main__":
    ...
    def _process(N_counter, draw_counter, repeat_counter, m):
        ...
        if m.LINK == HBM:
            ...
            """ Build model """
            config = Config(toml_path=toml_path)
            config.BUILD_DIR = os.path.join(CONFIG.BUILD_DIR, prefix, m.LINK, prefix_2, draw_dir, N_dir, seed_dir)
        ...
        elif m.LINK == "non_hierarchical_bayesian" or m.LINK == "hb_iterated" or m.LINK == "all_in":
            for subject in subjects_ind:
                ...
                """ Build model """
                config = Config(toml_path=toml_path)
                config.BUILD_DIR = os.path.join(CONFIG.BUILD_DIR, prefix, m.LINK, prefix_2, draw_dir, N_dir, seed_dir, subject_dir)
                model = m(config=config)
