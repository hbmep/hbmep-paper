...
if __name__ == "__main__":
    ...
    def _process(N_counter, draw_counter, repeat_counter, m):
        ...
        if m.LINK == "hierarchical_bayesian":
            ...
            """ Build model """
            config = Config(toml_path=toml_path)
            config.BUILD_DIR = os.path.join(CONFIG.BUILD_DIR, simulation_prefix, experiment_prefix, m.LINK, draw_dir, N_dir, seed_dir)
            ...
            for subject in subjects_ind:
                ...
                """ Build model """
                config = Config(toml_path=toml_path)
                config.BUILD_DIR = os.path.join(CONFIG.BUILD_DIR, simulation_prefix, experiment_prefix, m.LINK, draw_dir, N_dir, seed_dir, subject_dir)
                model = m(config=config)
