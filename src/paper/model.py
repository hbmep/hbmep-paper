...
class HB(BaseModel):
    ...
    def constln_l5(self, intensity, features, response=None, **kw):
        num_data = intensity.shape[0]
        num_features = np.max(features, axis=0) + 1
        run_id = self.run_id
    ...
    def constln_l4(self, intensity, features, response=None, **kw):
        num_data = intensity.shape[0]
        num_features = np.max(features, axis=0) + 1
        run_id = self.run_id
    ...
    def constln_rlin(self, intensity, features, response=None, **kw):
        num_data = intensity.shape[0]
        num_features = np.max(features, axis=0) + 1
        run_id = self.run_id
