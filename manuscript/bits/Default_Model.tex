...
\begin{figure}[H]
    ...
    \tikz{ %
        % Obs
        \node[obs,fill,] (obs) {${y_i}^{p,m}$};%

        % Deterministic mu, beta
        \node[det,above=of obs,xshift=-1.2cm,yshift=-.5cm,fill,] (beta) {${\beta_i}^{p,m}$}; %
        \node[det,above=of obs,xshift=1.2cm,yshift=.2cm,fill,] (mu) {${\mu_i}^{p,m}$}; %
        \node[obs,above=of obs,xshift=-.8cm,yshift=1.5cm,fill,] (intensity) {${x_i}^{p,m}$};%
        \plate[inner sep=.1cm, yshift=.07cm] {plate_obs} {(obs)(mu)(beta)(intensity)} {$\text{intensity }i: 1, 2 \ldots n(p)$};

        % % PRIORS
        % a, b, v
        \node[latent,above=of obs,yshift=3cm,xshift=-1cm,fill,] (a) {$a^{p,m}$};
        \node[latent,right=of a,xshift=-.3cm,fill,] (b) {$b^{p,m}$};
        % \node[latent,right=of b,xshift=-.9cm,fill,] (v) {$v^{p}$};
        % L, ell, H
        \node[latent,right=of b,xshift=-.9cm,fill,] (L) {$L^{p,m}$};
        \node[latent,right=of L,xshift=-.9cm,fill,] (ell) {$\ell^{p,m}$};
        \node[latent,right=of ell,xshift=-.9cm,fill,] (H) {$H^{p,m}$};
        % g1, g2
        \node[latent,left=of a,xshift=.3cm,fill,] (g2) {${c_2}^{p,m}$};
        \node[latent,left=of g2,xshift=.9cm,fill,] (g1) {${c_1}^{p,m}$};
        ...
        \edge {beta,mu} {obs};
        \edge {mu} {beta};
        \plate[inner sep=.1cm, yshift=.07cm] {plate_muscle} {(plate_participant)} {$\text{muscle }m : 1, 2 \ldots N_M$};
    }
\end{figure}
